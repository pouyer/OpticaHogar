<?php
    /**
     * Modelo para la tabla tipos_origen_enfermedad     */
require_once '../conexion.php';

class ModeloTipos_origen_enfermedad {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM tipos_origen_enfermedad";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM tipos_origen_enfermedad ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `tipos_origen_enfermedad`.`id`, `tipos_origen_enfermedad`.`codigo`, `tipos_origen_enfermedad`.`nombre`, `tipos_origen_enfermedad`.`descripcion`, `tipos_origen_enfermedad`.`estado`, `tipos_origen_enfermedad`.`usuario_id_inserto`, `tipos_origen_enfermedad`.`fecha_insercion`, `tipos_origen_enfermedad`.`usuario_id_actualizo`, `tipos_origen_enfermedad`.`fecha_actualizacion`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`tipos_origen_enfermedad`.`id`', '`tipos_origen_enfermedad`.`codigo`', '`tipos_origen_enfermedad`.`nombre`', '`tipos_origen_enfermedad`.`descripcion`', '`tipos_origen_enfermedad`.`estado`', '`tipos_origen_enfermedad`.`usuario_id_inserto`', '`tipos_origen_enfermedad`.`fecha_insercion`', '`tipos_origen_enfermedad`.`usuario_id_actualizo`', '`tipos_origen_enfermedad`.`fecha_actualizacion`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `tipos_origen_enfermedad`.`estado` ASC, `tipos_origen_enfermedad`.`nombre` ASC, `tipos_origen_enfermedad`.`codigo` ASC ";
        }

        $query = "SELECT `tipos_origen_enfermedad`.*  FROM tipos_origen_enfermedad";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `tipos_origen_enfermedad`.*  FROM tipos_origen_enfermedad";
        $query .= " WHERE `tipos_origen_enfermedad`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO tipos_origen_enfermedad (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE tipos_origen_enfermedad SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM tipos_origen_enfermedad WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`tipos_origen_enfermedad`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `tipos_origen_enfermedad`.`codigo`, `tipos_origen_enfermedad`.`nombre`, `tipos_origen_enfermedad`.`descripcion`, `tipos_origen_enfermedad`.`estado`, `tipos_origen_enfermedad`.`usuario_id_inserto`, `tipos_origen_enfermedad`.`fecha_insercion`, `tipos_origen_enfermedad`.`usuario_id_actualizo`, `tipos_origen_enfermedad`.`fecha_actualizacion` FROM tipos_origen_enfermedad";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`tipos_origen_enfermedad`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
