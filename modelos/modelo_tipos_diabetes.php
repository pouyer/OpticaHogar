<?php
    /**
     * Modelo para la tabla tipos_diabetes     */
require_once '../conexion.php';

class ModeloTipos_diabetes {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM tipos_diabetes";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM tipos_diabetes ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `tipos_diabetes`.`id`, `tipos_diabetes`.`codigo`, `tipos_diabetes`.`nombre`, `tipos_diabetes`.`descripcion`, `tipos_diabetes`.`estado`, `tipos_diabetes`.`usuario_id_inserto`, `tipos_diabetes`.`fecha_insercion`, `tipos_diabetes`.`usuario_id_actualizo`, `tipos_diabetes`.`fecha_actualizacion`, `tipos_diabetes`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`tipos_diabetes`.`id`', '`tipos_diabetes`.`codigo`', '`tipos_diabetes`.`nombre`', '`tipos_diabetes`.`descripcion`', '`tipos_diabetes`.`estado`', '`tipos_diabetes`.`usuario_id_inserto`', '`tipos_diabetes`.`fecha_insercion`', '`tipos_diabetes`.`usuario_id_actualizo`', '`tipos_diabetes`.`fecha_actualizacion`', '`tipos_diabetes`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `tipos_diabetes`.`estado` ASC, `tipos_diabetes`.`orden` ASC, `tipos_diabetes`.`codigo` ASC ";
        }

        $query = "SELECT `tipos_diabetes`.*  FROM tipos_diabetes";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `tipos_diabetes`.*  FROM tipos_diabetes";
        $query .= " WHERE `tipos_diabetes`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO tipos_diabetes (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE tipos_diabetes SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM tipos_diabetes WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`tipos_diabetes`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `tipos_diabetes`.`codigo`, `tipos_diabetes`.`nombre`, `tipos_diabetes`.`descripcion`, `tipos_diabetes`.`estado`, `tipos_diabetes`.`usuario_id_inserto`, `tipos_diabetes`.`fecha_insercion`, `tipos_diabetes`.`usuario_id_actualizo`, `tipos_diabetes`.`fecha_actualizacion`, `tipos_diabetes`.`orden` FROM tipos_diabetes";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`tipos_diabetes`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
