<?php
    /**
     * Modelo para la tabla tipos_profesional     */
require_once '../conexion.php';

class ModeloTipos_profesional {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM tipos_profesional";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM tipos_profesional ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `tipos_profesional`.`id`, `tipos_profesional`.`codigo`, `tipos_profesional`.`nombre`, `tipos_profesional`.`descripcion`, `tipos_profesional`.`estado`, `tipos_profesional`.`usuario_id_inserto`, `tipos_profesional`.`fecha_insercion`, `tipos_profesional`.`usuario_id_actualizo`, `tipos_profesional`.`fecha_actualizacion`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`tipos_profesional`.`id`', '`tipos_profesional`.`codigo`', '`tipos_profesional`.`nombre`', '`tipos_profesional`.`descripcion`', '`tipos_profesional`.`estado`', '`tipos_profesional`.`usuario_id_inserto`', '`tipos_profesional`.`fecha_insercion`', '`tipos_profesional`.`usuario_id_actualizo`', '`tipos_profesional`.`fecha_actualizacion`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `tipos_profesional`.`estado` ASC, `tipos_profesional`.`codigo` ASC, `tipos_profesional`.`nombre` ASC ";
        }

        $query = "SELECT `tipos_profesional`.*  FROM tipos_profesional";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `tipos_profesional`.*  FROM tipos_profesional";
        $query .= " WHERE `tipos_profesional`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO tipos_profesional (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE tipos_profesional SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM tipos_profesional WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        $simpleCols = ['id', 'codigo', 'nombre', 'descripcion', 'estado', 'usuario_id_inserto', 'fecha_insercion', 'usuario_id_actualizo', 'fecha_actualizacion'];
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`tipos_profesional`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `tipos_profesional`.`codigo`, `tipos_profesional`.`nombre`, `tipos_profesional`.`descripcion`, `tipos_profesional`.`estado`, `tipos_profesional`.`usuario_id_inserto`, `tipos_profesional`.`fecha_insercion`, `tipos_profesional`.`usuario_id_actualizo`, `tipos_profesional`.`fecha_actualizacion` FROM tipos_profesional";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`tipos_profesional`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
