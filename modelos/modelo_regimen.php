...
class ModeloRegimen {
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM regimen";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM regimen ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `regimen`.`id`, `regimen`.`codigo`, `regimen`.`nombre`, `regimen`.`campo_ordenamiento`, `regimen`.`estado`, `regimen`.`usuario_id_inserto`, `regimen`.`fecha_insercion`, `regimen`.`usuario_id_actualizo`, `regimen`.`fecha_actualizacion`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`regimen`.`id`', '`regimen`.`codigo`', '`regimen`.`nombre`', '`regimen`.`campo_ordenamiento`', '`regimen`.`estado`', '`regimen`.`usuario_id_inserto`', '`regimen`.`fecha_insercion`', '`regimen`.`usuario_id_actualizo`', '`regimen`.`fecha_actualizacion`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `regimen`.`estado` ASC, `regimen`.`campo_ordenamiento` ASC, `regimen`.`nombre` ASC ";
        }

        $query = "SELECT `regimen`.*  FROM regimen";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `regimen`.*  FROM regimen";
        $query .= " WHERE `regimen`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO regimen (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE regimen SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM regimen WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `regimen`.`id`, `regimen`.`codigo`, `regimen`.`nombre`, `regimen`.`campo_ordenamiento`, `regimen`.`estado`, `regimen`.`usuario_id_inserto`, `regimen`.`fecha_insercion`, `regimen`.`usuario_id_actualizo`, `regimen`.`fecha_actualizacion` FROM regimen";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`regimen`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
