<?php
    /**
     * Modelo para la tabla estados_paciente     */
require_once '../conexion.php';

class ModeloEstados_paciente {
    private $conexion;
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `estados_paciente`.*  FROM estados_paciente";
        $query .= " WHERE `estados_paciente`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO estados_paciente (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE estados_paciente SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM estados_paciente WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`estados_paciente`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`estados_paciente`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
