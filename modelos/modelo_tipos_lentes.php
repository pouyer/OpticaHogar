<?php
    /**
     * Modelo para la tabla tipos_lentes     */
require_once '../conexion.php';

class ModeloTipos_lentes {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM tipos_lentes";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM tipos_lentes ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `tipos_lentes`.`id`, `tipos_lentes`.`codigo`, `tipos_lentes`.`nombre`, `tipos_lentes`.`descripcion`, `tipos_lentes`.`estado`, `tipos_lentes`.`usuario_id_inserto`, `tipos_lentes`.`fecha_insercion`, `tipos_lentes`.`usuario_id_actualizo`, `tipos_lentes`.`fecha_actualizacion`, `tipos_lentes`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`tipos_lentes`.`id`', '`tipos_lentes`.`codigo`', '`tipos_lentes`.`nombre`', '`tipos_lentes`.`descripcion`', '`tipos_lentes`.`estado`', '`tipos_lentes`.`usuario_id_inserto`', '`tipos_lentes`.`fecha_insercion`', '`tipos_lentes`.`usuario_id_actualizo`', '`tipos_lentes`.`fecha_actualizacion`', '`tipos_lentes`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `tipos_lentes`.`estado` ASC, `tipos_lentes`.`orden` ASC, `tipos_lentes`.`codigo` ASC ";
        }

        $query = "SELECT `tipos_lentes`.*  FROM tipos_lentes";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `tipos_lentes`.*  FROM tipos_lentes";
        $query .= " WHERE `tipos_lentes`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO tipos_lentes (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE tipos_lentes SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM tipos_lentes WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `tipos_lentes`.`codigo`, `tipos_lentes`.`nombre`, `tipos_lentes`.`descripcion`, `tipos_lentes`.`estado`, `tipos_lentes`.`usuario_id_inserto`, `tipos_lentes`.`fecha_insercion`, `tipos_lentes`.`usuario_id_actualizo`, `tipos_lentes`.`fecha_actualizacion`, `tipos_lentes`.`orden` FROM tipos_lentes";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`tipos_lentes`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
