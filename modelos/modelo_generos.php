...
class ModeloGeneros {
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM generos";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM generos ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `generos`.`id`, `generos`.`codigo`, `generos`.`nombre`, `generos`.`estado`, `generos`.`usuario_id_inserto`, `generos`.`fecha_insercion`, `generos`.`usuario_id_actualizo`, `generos`.`fecha_actualizacion`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`generos`.`id`', '`generos`.`codigo`', '`generos`.`nombre`', '`generos`.`estado`', '`generos`.`usuario_id_inserto`', '`generos`.`fecha_insercion`', '`generos`.`usuario_id_actualizo`', '`generos`.`fecha_actualizacion`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `generos`.`estado` ASC, `generos`.`codigo` ASC, `generos`.`nombre` ASC ";
        }

        $query = "SELECT `generos`.*  FROM generos";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `generos`.*  FROM generos";
        $query .= " WHERE `generos`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO generos (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE generos SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM generos WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        $simpleCols = ['id', 'codigo', 'nombre', 'estado', 'usuario_id_inserto', 'fecha_insercion', 'usuario_id_actualizo', 'fecha_actualizacion'];
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `generos`.`codigo`, `generos`.`nombre`, `generos`.`estado`, `generos`.`usuario_id_inserto`, `generos`.`fecha_insercion`, `generos`.`usuario_id_actualizo`, `generos`.`fecha_actualizacion` FROM generos";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`generos`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
