<?php
    /**
     * Modelo para la tabla ocupaciones     */
require_once '../conexion.php';

class ModeloOcupaciones {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM ocupaciones";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM ocupaciones ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `ocupaciones`.`id`, `ocupaciones`.`nombre`, `ocupaciones`.`descripcion`, `ocupaciones`.`estado`, `ocupaciones`.`usuario_id_inserto`, `ocupaciones`.`fecha_insercion`, `ocupaciones`.`usuario_id_actualizo`, `ocupaciones`.`fecha_actualizacion`, `ocupaciones`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`ocupaciones`.`id`', '`ocupaciones`.`nombre`', '`ocupaciones`.`descripcion`', '`ocupaciones`.`estado`', '`ocupaciones`.`usuario_id_inserto`', '`ocupaciones`.`fecha_insercion`', '`ocupaciones`.`usuario_id_actualizo`', '`ocupaciones`.`fecha_actualizacion`', '`ocupaciones`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `ocupaciones`.`estado` ASC, `ocupaciones`.`orden` ASC, `ocupaciones`.`nombre` ASC ";
        }

        $query = "SELECT `ocupaciones`.*  FROM ocupaciones";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `ocupaciones`.*  FROM ocupaciones";
        $query .= " WHERE `ocupaciones`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO ocupaciones (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE ocupaciones SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM ocupaciones WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        $simpleCols = ['id', 'nombre', 'descripcion', 'estado', 'usuario_id_inserto', 'fecha_insercion', 'usuario_id_actualizo', 'fecha_actualizacion', 'orden'];
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `ocupaciones`.`nombre`, `ocupaciones`.`descripcion`, `ocupaciones`.`estado`, `ocupaciones`.`usuario_id_inserto`, `ocupaciones`.`fecha_insercion`, `ocupaciones`.`usuario_id_actualizo`, `ocupaciones`.`fecha_actualizacion`, `ocupaciones`.`orden` FROM ocupaciones";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`ocupaciones`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
