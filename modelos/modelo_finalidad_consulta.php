<?php
    /**
     * Modelo para la tabla finalidad_consulta     */
require_once '../conexion.php';

class ModeloFinalidad_consulta {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM finalidad_consulta";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM finalidad_consulta ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `finalidad_consulta`.`id`, `finalidad_consulta`.`codigo`, `finalidad_consulta`.`nombre`, `finalidad_consulta`.`campo_ordenamiento`, `finalidad_consulta`.`estado`, `finalidad_consulta`.`usuario_id_inserto`, `finalidad_consulta`.`fecha_insercion`, `finalidad_consulta`.`usuario_id_actualizo`, `finalidad_consulta`.`fecha_actualizacion`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`finalidad_consulta`.`id`', '`finalidad_consulta`.`codigo`', '`finalidad_consulta`.`nombre`', '`finalidad_consulta`.`campo_ordenamiento`', '`finalidad_consulta`.`estado`', '`finalidad_consulta`.`usuario_id_inserto`', '`finalidad_consulta`.`fecha_insercion`', '`finalidad_consulta`.`usuario_id_actualizo`', '`finalidad_consulta`.`fecha_actualizacion`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `finalidad_consulta`.`estado` ASC, `finalidad_consulta`.`campo_ordenamiento` ASC, `finalidad_consulta`.`nombre` ASC ";
        }

        $query = "SELECT `finalidad_consulta`.*  FROM finalidad_consulta";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `finalidad_consulta`.*  FROM finalidad_consulta";
        $query .= " WHERE `finalidad_consulta`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO finalidad_consulta (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE finalidad_consulta SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM finalidad_consulta WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`finalidad_consulta`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `finalidad_consulta`.`id`, `finalidad_consulta`.`codigo`, `finalidad_consulta`.`nombre`, `finalidad_consulta`.`campo_ordenamiento`, `finalidad_consulta`.`estado`, `finalidad_consulta`.`usuario_id_inserto`, `finalidad_consulta`.`fecha_insercion`, `finalidad_consulta`.`usuario_id_actualizo`, `finalidad_consulta`.`fecha_actualizacion` FROM finalidad_consulta";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`finalidad_consulta`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
