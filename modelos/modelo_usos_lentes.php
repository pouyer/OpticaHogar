<?php
    /**
     * Modelo para la tabla usos_lentes     */
require_once '../conexion.php';

class ModeloUsos_lentes {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM usos_lentes";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM usos_lentes ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `usos_lentes`.`id`, `usos_lentes`.`codigo`, `usos_lentes`.`nombre`, `usos_lentes`.`descripcion`, `usos_lentes`.`estado`, `usos_lentes`.`usuario_id_inserto`, `usos_lentes`.`fecha_insercion`, `usos_lentes`.`usuario_id_actualizo`, `usos_lentes`.`fecha_actualizacion`, `usos_lentes`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`usos_lentes`.`id`', '`usos_lentes`.`codigo`', '`usos_lentes`.`nombre`', '`usos_lentes`.`descripcion`', '`usos_lentes`.`estado`', '`usos_lentes`.`usuario_id_inserto`', '`usos_lentes`.`fecha_insercion`', '`usos_lentes`.`usuario_id_actualizo`', '`usos_lentes`.`fecha_actualizacion`', '`usos_lentes`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `usos_lentes`.`estado` ASC, `usos_lentes`.`orden` ASC, `usos_lentes`.`codigo` ASC ";
        }

        $query = "SELECT `usos_lentes`.*  FROM usos_lentes";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `usos_lentes`.*  FROM usos_lentes";
        $query .= " WHERE `usos_lentes`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO usos_lentes (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE usos_lentes SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM usos_lentes WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `usos_lentes`.`codigo`, `usos_lentes`.`nombre`, `usos_lentes`.`descripcion`, `usos_lentes`.`estado`, `usos_lentes`.`usuario_id_inserto`, `usos_lentes`.`fecha_insercion`, `usos_lentes`.`usuario_id_actualizo`, `usos_lentes`.`fecha_actualizacion`, `usos_lentes`.`orden` FROM usos_lentes";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`usos_lentes`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
