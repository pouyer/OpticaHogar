<?php
    /**
     * Modelo para la tabla tipos_identificacion     */
require_once '../conexion.php';

class ModeloTipos_identificacion {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM tipos_identificacion";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM tipos_identificacion ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `tipos_identificacion`.`id`, `tipos_identificacion`.`codigo`, `tipos_identificacion`.`nombre`, `tipos_identificacion`.`descripcion`, `tipos_identificacion`.`estado`, `tipos_identificacion`.`usuario_id_inserto`, `tipos_identificacion`.`fecha_insercion`, `tipos_identificacion`.`usuario_id_actualizo`, `tipos_identificacion`.`fecha_actualizacion`, `tipos_identificacion`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`tipos_identificacion`.`id`', '`tipos_identificacion`.`codigo`', '`tipos_identificacion`.`nombre`', '`tipos_identificacion`.`descripcion`', '`tipos_identificacion`.`estado`', '`tipos_identificacion`.`usuario_id_inserto`', '`tipos_identificacion`.`fecha_insercion`', '`tipos_identificacion`.`usuario_id_actualizo`', '`tipos_identificacion`.`fecha_actualizacion`', '`tipos_identificacion`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `tipos_identificacion`.`estado` ASC, `tipos_identificacion`.`orden` ASC, `tipos_identificacion`.`codigo` ASC ";
        }

        $query = "SELECT `tipos_identificacion`.*  FROM tipos_identificacion";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `tipos_identificacion`.*  FROM tipos_identificacion";
        $query .= " WHERE `tipos_identificacion`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO tipos_identificacion (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE tipos_identificacion SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM tipos_identificacion WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`tipos_identificacion`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `tipos_identificacion`.`codigo`, `tipos_identificacion`.`nombre`, `tipos_identificacion`.`descripcion`, `tipos_identificacion`.`estado`, `tipos_identificacion`.`usuario_id_inserto`, `tipos_identificacion`.`fecha_insercion`, `tipos_identificacion`.`usuario_id_actualizo`, `tipos_identificacion`.`fecha_actualizacion`, `tipos_identificacion`.`orden` FROM tipos_identificacion";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`tipos_identificacion`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
