<?php
    /**
     * Modelo para la tabla tipos_consulta     */
require_once '../conexion.php';

class ModeloTipos_consulta {
    private $conexion;
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `tipos_consulta`.`estado` ASC, `tipos_consulta`.`orden` ASC, `tipos_consulta`.`nombre` ASC ";
        }
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `tipos_consulta`.*  FROM tipos_consulta";
        $query .= " WHERE `tipos_consulta`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO tipos_consulta (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE tipos_consulta SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM tipos_consulta WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`tipos_consulta`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`tipos_consulta`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
