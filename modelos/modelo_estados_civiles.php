<?php
    /**
     * Modelo para la tabla estados_civiles     */
require_once '../conexion.php';

class ModeloEstados_civiles {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM estados_civiles";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM estados_civiles ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `estados_civiles`.`id`, `estados_civiles`.`nombre`, `estados_civiles`.`estado`, `estados_civiles`.`usuario_id_inserto`, `estados_civiles`.`fecha_insercion`, `estados_civiles`.`usuario_id_actualizo`, `estados_civiles`.`fecha_actualizacion`, `estados_civiles`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`estados_civiles`.`id`', '`estados_civiles`.`nombre`', '`estados_civiles`.`estado`', '`estados_civiles`.`usuario_id_inserto`', '`estados_civiles`.`fecha_insercion`', '`estados_civiles`.`usuario_id_actualizo`', '`estados_civiles`.`fecha_actualizacion`', '`estados_civiles`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `estados_civiles`.`estado` ASC, `estados_civiles`.`orden` ASC, `estados_civiles`.`nombre` ASC ";
        }

        $query = "SELECT `estados_civiles`.*  FROM estados_civiles";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `estados_civiles`.*  FROM estados_civiles";
        $query .= " WHERE `estados_civiles`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO estados_civiles (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE estados_civiles SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM estados_civiles WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        $simpleCols = ['id', 'nombre', 'estado', 'usuario_id_inserto', 'fecha_insercion', 'usuario_id_actualizo', 'fecha_actualizacion', 'orden'];
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`estados_civiles`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `estados_civiles`.`nombre`, `estados_civiles`.`estado`, `estados_civiles`.`usuario_id_inserto`, `estados_civiles`.`fecha_insercion`, `estados_civiles`.`usuario_id_actualizo`, `estados_civiles`.`fecha_actualizacion`, `estados_civiles`.`orden` FROM estados_civiles";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`estados_civiles`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
