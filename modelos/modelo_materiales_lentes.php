<?php
    /**
     * Modelo para la tabla materiales_lentes     */
require_once '../conexion.php';

class ModeloMateriales_lentes {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM materiales_lentes";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM materiales_lentes ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `materiales_lentes`.`id`, `materiales_lentes`.`codigo`, `materiales_lentes`.`nombre`, `materiales_lentes`.`descripcion`, `materiales_lentes`.`estado`, `materiales_lentes`.`usuario_id_inserto`, `materiales_lentes`.`fecha_insercion`, `materiales_lentes`.`usuario_id_actualizo`, `materiales_lentes`.`fecha_actualizacion`, `materiales_lentes`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`materiales_lentes`.`id`', '`materiales_lentes`.`codigo`', '`materiales_lentes`.`nombre`', '`materiales_lentes`.`descripcion`', '`materiales_lentes`.`estado`', '`materiales_lentes`.`usuario_id_inserto`', '`materiales_lentes`.`fecha_insercion`', '`materiales_lentes`.`usuario_id_actualizo`', '`materiales_lentes`.`fecha_actualizacion`', '`materiales_lentes`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `materiales_lentes`.`estado` ASC, `materiales_lentes`.`orden` ASC, `materiales_lentes`.`nombre` ASC ";
        }

        $query = "SELECT `materiales_lentes`.*  FROM materiales_lentes";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `materiales_lentes`.*  FROM materiales_lentes";
        $query .= " WHERE `materiales_lentes`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO materiales_lentes (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE materiales_lentes SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM materiales_lentes WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`materiales_lentes`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `materiales_lentes`.`codigo`, `materiales_lentes`.`nombre`, `materiales_lentes`.`descripcion`, `materiales_lentes`.`estado`, `materiales_lentes`.`usuario_id_inserto`, `materiales_lentes`.`fecha_insercion`, `materiales_lentes`.`usuario_id_actualizo`, `materiales_lentes`.`fecha_actualizacion`, `materiales_lentes`.`orden` FROM materiales_lentes";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`materiales_lentes`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
