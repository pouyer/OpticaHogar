<?php
    /**
     * Modelo para la tabla parentescos     */
...
class ModeloParentescos {
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM parentescos";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM parentescos ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `parentescos`.`id`, `parentescos`.`nombre`, `parentescos`.`estado`, `parentescos`.`usuario_id_inserto`, `parentescos`.`fecha_insercion`, `parentescos`.`usuario_id_actualizo`, `parentescos`.`fecha_actualizacion`, `parentescos`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`parentescos`.`id`', '`parentescos`.`nombre`', '`parentescos`.`estado`', '`parentescos`.`usuario_id_inserto`', '`parentescos`.`fecha_insercion`', '`parentescos`.`usuario_id_actualizo`', '`parentescos`.`fecha_actualizacion`', '`parentescos`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `parentescos`.`estado` ASC, `parentescos`.`orden` ASC, `parentescos`.`nombre` ASC ";
        }

        $query = "SELECT `parentescos`.*  FROM parentescos";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `parentescos`.*  FROM parentescos";
        $query .= " WHERE `parentescos`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO parentescos (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE parentescos SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM parentescos WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `parentescos`.`nombre`, `parentescos`.`estado`, `parentescos`.`usuario_id_inserto`, `parentescos`.`fecha_insercion`, `parentescos`.`usuario_id_actualizo`, `parentescos`.`fecha_actualizacion`, `parentescos`.`orden` FROM parentescos";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`parentescos`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
