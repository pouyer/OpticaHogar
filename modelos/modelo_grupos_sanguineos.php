<?php
    /**
     * Modelo para la tabla grupos_sanguineos     */
require_once '../conexion.php';

class ModeloGrupos_sanguineos {
    private $conexion;
    ...
    public function contarRegistros() {
        $query = "SELECT COUNT(*) as total FROM grupos_sanguineos";
    ...
    public function contarRegistrosPorBusqueda($termino) {
        $query = "SELECT COUNT(*) as total FROM grupos_sanguineos ";
        $query .= " WHERE ";
        $query .= "CONCAT_WS(' ', `grupos_sanguineos`.`id`, `grupos_sanguineos`.`codigo`, `grupos_sanguineos`.`nombre`, `grupos_sanguineos`.`descripcion`, `grupos_sanguineos`.`estado`, `grupos_sanguineos`.`usuario_id_inserto`, `grupos_sanguineos`.`fecha_insercion`, `grupos_sanguineos`.`usuario_id_actualizo`, `grupos_sanguineos`.`fecha_actualizacion`, `grupos_sanguineos`.`orden`) LIKE ?";
        $stmt = $this->conexion->prepare($query);
        $termino = "%" . $termino . "%";
    ...
    public function obtenerTodos($registrosPorPagina, $offset, $orderBy = null, $orderDir = 'DESC') {
        ...
        $allowedColumns = ['`grupos_sanguineos`.`id`', '`grupos_sanguineos`.`codigo`', '`grupos_sanguineos`.`nombre`', '`grupos_sanguineos`.`descripcion`', '`grupos_sanguineos`.`estado`', '`grupos_sanguineos`.`usuario_id_inserto`', '`grupos_sanguineos`.`fecha_insercion`', '`grupos_sanguineos`.`usuario_id_actualizo`', '`grupos_sanguineos`.`fecha_actualizacion`', '`grupos_sanguineos`.`orden`'];
        ...
        if (empty($orderSQL)) {
            ...
            $orderSQL = " ORDER BY `grupos_sanguineos`.`estado` ASC, `grupos_sanguineos`.`orden` ASC, `grupos_sanguineos`.`codigo` ASC ";
        }

        $query = "SELECT `grupos_sanguineos`.*  FROM grupos_sanguineos";
        $query .= $orderSQL;
        $query .= " LIMIT ? OFFSET ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function obtenerPorId($id) {
        $query = "SELECT `grupos_sanguineos`.*  FROM grupos_sanguineos";
        $query .= " WHERE `grupos_sanguineos`.$this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function crear($datos) {
        ...
        $query = "INSERT INTO grupos_sanguineos (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $valores) . ")";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function actualizar($id, $datos) {
        ...
        $params[] = $id;
        $tipos .= $tipos_pk;
        $query = "UPDATE grupos_sanguineos SET " . implode(', ', $actualizaciones) . " WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        if (!empty($params)) {
    ...
    public function eliminar($id) {
        $query = "DELETE FROM grupos_sanguineos WHERE $this->llavePrimaria = ?";
        $stmt = $this->conexion->prepare($query);
        $stmt->bind_param('i', $id);
        return $stmt->execute();
    }
    ...
    public function contarPorCampo($campo, $valor) {
        ...
        foreach ($simpleCols as $idx => $sc) {
            if ($sc === $campo) {
                 $esValido = true;
                 $columnaSQL = "`grupos_sanguineos`.`" . $campo . "`";
                 break;
            }
        }
        if (!$esValido) {
    ...
    public function exportarDatos($termino = '', $campoFiltro = '') {
        try {
            $query = "SELECT `grupos_sanguineos`.`codigo`, `grupos_sanguineos`.`nombre`, `grupos_sanguineos`.`descripcion`, `grupos_sanguineos`.`estado`, `grupos_sanguineos`.`usuario_id_inserto`, `grupos_sanguineos`.`fecha_insercion`, `grupos_sanguineos`.`usuario_id_actualizo`, `grupos_sanguineos`.`fecha_actualizacion`, `grupos_sanguineos`.`orden` FROM grupos_sanguineos";
            $query .= " WHERE ";
            ...
            if (!empty($campoFiltro)) {
                ...
                foreach ($simpleCols as $idx => $sc) {
                    if ($sc === $campoFiltro) {
                         ...
                         $columnaSQL = "`grupos_sanguineos`.`" . $campoFiltro . "`";
                         break;
                    }
                }
                if (!$usarFiltroCampo) {
