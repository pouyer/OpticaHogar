...
class ControladorTipos_diabetes {
    private $modelo;
    private $modeloLog;
    private $es_vista;
    ...
    public function __construct() {
        ...
        $this->modelo = new ModeloTipos_diabetes();
        $this->modeloLog = new ModeloAcc_log();
        $this->es_vista = false;
        
        // Cargar permisos
        $this->permisos = $_SESSION['permisos']['vista_tipos_diabetes.php'] ?? ['ins' => 0, 'upd' => 0, 'del' => 0, 'exp' => 0];
    }
    ...
    public function crear($datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'CREATE', 'tipos_diabetes', 'Nuevo registro creado');
        }
        return $resultado;
    }
    ...
    public function actualizar($id, $datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'UPDATE', 'tipos_diabetes', 'Registro actualizado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function eliminar($id) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'DELETE', 'tipos_diabetes', 'Registro eliminado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function exportar($formato) {
        ...
        try {
            ...
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'EXPORT', 'tipos_diabetes', $logMsg);
            ...
            $filename = "tipos_diabetes_export_{$timestamp}";
            if (!empty($termino)) {
...
$accion = $_GET['action'] ?? '';
$controlador = new ControladorTipos_diabetes();
...
switch ($accion) {
    ...
    case 'buscar':
        ...
        include '../vistas/vista_tipos_diabetes.php';
        break;
