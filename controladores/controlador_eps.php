...
class ControladorEps {
    ...
    public function __construct() {
        ...
        $this->modelo = new ModeloEps();
        $this->modeloLog = new ModeloAcc_log();
        $this->es_vista = false;
        
        // Cargar permisos
        $this->permisos = $_SESSION['permisos']['vista_eps.php'] ?? ['ins' => 0, 'upd' => 0, 'del' => 0, 'exp' => 0];
    }
    ...
    public function crear($datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'CREATE', 'eps', 'Nuevo registro creado');
        }
        return $resultado;
    }
    ...
    public function actualizar($id, $datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'UPDATE', 'eps', 'Registro actualizado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function eliminar($id) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'DELETE', 'eps', 'Registro eliminado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function exportar($formato) {
        ...
        try {
            ...
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'EXPORT', 'eps', $logMsg);
            ...
            $filename = "eps_export_{$timestamp}";
            if (!empty($termino)) {
...
$accion = $_GET['action'] ?? '';
$controlador = new ControladorEps();
