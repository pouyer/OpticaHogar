...
class ControladorGeneros {
    ...
    public function __construct() {
        ...
        // Cargar permisos
        $this->permisos = $_SESSION['permisos']['vista_generos.php'] ?? ['ins' => 0, 'upd' => 0, 'del' => 0, 'exp' => 0];
    }
    ...
    public function crear($datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'CREATE', 'generos', 'Nuevo registro creado');
        }
        return $resultado;
    }
    ...
    public function actualizar($id, $datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'UPDATE', 'generos', 'Registro actualizado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function eliminar($id) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'DELETE', 'generos', 'Registro eliminado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function exportar($formato) {
        ...
        try {
            ...
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'EXPORT', 'generos', $logMsg);
            ...
            $filename = "generos_export_{$timestamp}";
            if (!empty($termino)) {
...
switch ($accion) {
    ...
    case 'buscar':
        ...
        include '../vistas/vista_generos.php';
        break;
