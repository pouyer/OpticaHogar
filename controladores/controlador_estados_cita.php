...
class ControladorEstados_cita {
    ...
    public function __construct() {
        ...
        $this->modelo = new ModeloEstados_cita();
        $this->modeloLog = new ModeloAcc_log();
        $this->es_vista = false;
        
        // Cargar permisos
        $this->permisos = $_SESSION['permisos']['vista_estados_cita.php'] ?? ['ins' => 0, 'upd' => 0, 'del' => 0, 'exp' => 0];
    }
    ...
    public function crear($datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'CREATE', 'estados_cita', 'Nuevo registro creado');
        }
        return $resultado;
    }
    ...
    public function actualizar($id, $datos) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'UPDATE', 'estados_cita', 'Registro actualizado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function eliminar($id) {
        ...
        if ($resultado) {
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'DELETE', 'estados_cita', 'Registro eliminado ID: ' . $id);
        }
        return $resultado;
    }
    ...
    public function exportar($formato) {
        ...
        try {
            ...
            $this->modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'EXPORT', 'estados_cita', $logMsg);
            ...
            $filename = "estados_cita_export_{$timestamp}";
            if (!empty($termino)) {
...
$accion = $_GET['action'] ?? '';
$controlador = new ControladorEstados_cita();
...
switch ($accion) {
    ...
    case 'buscar':
        ...
        include '../vistas/vista_estados_cita.php';
        break;
