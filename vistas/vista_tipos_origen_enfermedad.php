...
$mi_programa = 'vista_tipos_origen_enfermedad.php'; // Debe coincidir con el nombre_archivo en acc_programa
...
<head>
    ...
    <title>Tipos_origen_enfermedad</title>
    ...
    <div class="container pb-5">
        ...
        <div class="main-card">
            <div class="card-header-custom d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="mb-0"><i class="icon-table me-2"></i> Tipos_origen_enfermedad</h3>
                ...
                <div class="d-flex gap-2">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                            <li><a class="dropdown-item" href="../controladores/controlador_tipos_origen_enfermedad.php?action=exportar&formato=excel&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-file-excel text-success me-2"></i> Excel</a></li>
                            <li><a class="dropdown-item" href="../controladores/controlador_tipos_origen_enfermedad.php?action=exportar&formato=csv&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-file-text text-primary me-2"></i> CSV</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item" href="../controladores/controlador_tipos_origen_enfermedad.php?action=exportar&formato=txt&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-doc-text-inv text-secondary me-2"></i> TXT</a></li>
                        </ul>
                    </div>
                    <?php endif; ?>
            ...
            <div class="card-body p-4">
                <!-- Buscador -->
                <form method="GET" action="vista_tipos_origen_enfermedad.php" class="mb-4">
                    <div class="input-group">
                        ...
                        <?php if(isset($_GET['busqueda']) && $_GET['busqueda'] !== ''): ?>
                            <a href="vista_tipos_origen_enfermedad.php" class="btn btn-outline-danger d-flex align-items-center"><i class="icon-cancel"></i></a>
                        <?php endif; ?>
                    </div>
                </form>
...
?>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`tipos_origen_enfermedad`.`codigo`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        codigo                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`tipos_origen_enfermedad`.`codigo`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`tipos_origen_enfermedad`.`nombre`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        nombre                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`tipos_origen_enfermedad`.`nombre`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`tipos_origen_enfermedad`.`descripcion`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        descripcion                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`tipos_origen_enfermedad`.`descripcion`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`tipos_origen_enfermedad`.`estado`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        estado                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`tipos_origen_enfermedad`.`estado`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                        ...
                        <tbody>
                            <?php
                            require_once '../modelos/modelo_tipos_origen_enfermedad.php';
                            ...
                            $modelo = new ModeloTipos_origen_enfermedad();
                            $modeloLog = new ModeloAcc_log();
                            $modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'VIEW', 'tipos_origen_enfermedad', 'Acceso a la pantalla de listado');
                     ...
                     <div class="modal-header">
                         <div class="row">
                             <div class="form-group col-md-8">
                               <h5 class="modal-title">Actualizar Tipos_origen_enfermedad - ID: </h5>
                             </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('formCrear').addEventListener('submit', function(e) {
                ...
                fetch('../controladores/controlador_tipos_origen_enfermedad.php?action=crear', {
                    method: 'POST',
            ...
            document.getElementById('formActualizar').addEventListener('submit', function(e) {
                e.preventDefault();
                const formData = new FormData(this);
                fetch('../controladores/controlador_tipos_origen_enfermedad.php?action=actualizar', {
                    method: 'POST',
                    body: new URLSearchParams(formData)
                })
                .then(response => response.json())
        ...
        function eliminar(id) {
            if (confirm('¿Estás seguro de que deseas eliminar este registro?')) {
                fetch('../controladores/controlador_tipos_origen_enfermedad.php?action=eliminar', {
                    method: 'POST',
                    headers: {
