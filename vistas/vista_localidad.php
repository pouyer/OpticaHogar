...
$mi_programa = 'vista_localidad.php'; // Debe coincidir con el nombre_archivo en acc_programa
    ...
    <div class="container pb-5">
        ...
        <div class="main-card">
            <div class="card-header-custom d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="mb-0"><i class="icon-table me-2"></i> Localidad</h3>
                ...
                <div class="d-flex gap-2">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                            <li><a class="dropdown-item" href="../controladores/controlador_localidad.php?action=exportar&formato=excel&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-file-excel text-success me-2"></i> Excel</a></li>
                            <li><a class="dropdown-item" href="../controladores/controlador_localidad.php?action=exportar&formato=csv&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-file-text text-primary me-2"></i> CSV</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item" href="../controladores/controlador_localidad.php?action=exportar&formato=txt&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-doc-text-inv text-secondary me-2"></i> TXT</a></li>
                        </ul>
                    </div>
                    <?php endif; ?>
            ...
            <div class="card-body p-4">
                <!-- Buscador -->
                <form method="GET" action="vista_localidad.php" class="mb-4">
                    <div class="input-group">
                        ...
                        <?php if(isset($_GET['busqueda']) && $_GET['busqueda'] !== ''): ?>
                            <a href="vista_localidad.php" class="btn btn-outline-danger d-flex align-items-center"><i class="icon-cancel"></i></a>
                        <?php endif; ?>
                    </div>
                </form>
...
?>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`localidad`.`id`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        id                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`localidad`.`id`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`municipio`.`Nombre`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        id_municipio                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`municipio`.`Nombre`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`localidad`.`codigo_dane`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        codigo_dane                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`localidad`.`codigo_dane`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`localidad`.`Nombre`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        Nombre                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`localidad`.`Nombre`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`localidad`.`campo_ordenamiento`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        campo_ordenamiento                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`localidad`.`campo_ordenamiento`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`localidad`.`estado`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        estado                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`localidad`.`estado`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                        ...
                        <tbody>
                            <?php
                            require_once '../modelos/modelo_localidad.php';
                            ...
                            $modelo = new ModeloLocalidad();
                            $modeloLog = new ModeloAcc_log();
                            $modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'VIEW', 'localidad', 'Acceso a la pantalla de listado');
                ...
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($registro['id_municipio_display'] ?? $registro['id_municipio']); ?></td>
                    ...
                    <td class="text-center">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#modalActualizar" data-idActualizar="<?php echo $registro['id']; ?>"
                           ...
                           data-id_municipio="<?php echo htmlspecialchars($registro['id_municipio']); ?>"
                        ...
                        <form id="formCrear" method="post">
                            <div class="row">                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="id_municipio" name="id_municipio">
                                        ...
                                        <?php endif; ?>                                        <?php foreach ($modelo->obtenerRelacionado_id_municipio() as $opcion): ?>                                        <option value="<?= $opcion['id'] ?>"><?= htmlspecialchars($opcion['texto']) ?></option>
                     ...
                     <div class="modal-header">
                         <div class="row">
                             <div class="form-group col-md-8">
                               <h5 class="modal-title">Actualizar Localidad - ID: </h5>
                             </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('formCrear').addEventListener('submit', function(e) {
                ...
                fetch('../controladores/controlador_localidad.php?action=crear', {
                    method: 'POST',
            ...
            modalActualizarElement.addEventListener('show.bs.modal', function(event) {
                ...
                var valorid_municipio = button.getAttribute('data-id_municipio');
                var inputid_municipio = modal.querySelector('#id_municipio');
                if(inputid_municipio) {
                    if (inputid_municipio.type === 'checkbox') {
                        inputid_municipio.checked = (valorid_municipio === 'activo');
                    } else {
                        inputid_municipio.value = valorid_municipio;
                    }
                }
            ...
            document.getElementById('formActualizar').addEventListener('submit', function(e) {
                e.preventDefault();
                const formData = new FormData(this);
                fetch('../controladores/controlador_localidad.php?action=actualizar', {
                    method: 'POST',
                    body: new URLSearchParams(formData)
                })
                .then(response => response.json())
        ...
        function eliminar(id) {
            if (confirm('¿Estás seguro de que deseas eliminar este registro?')) {
                fetch('../controladores/controlador_localidad.php?action=eliminar', {
                    method: 'POST',
                    headers: {
