...
$mi_programa = 'vista_parentescos.php'; // Debe coincidir con el nombre_archivo en acc_programa
    ...
    <div class="container pb-5">
        ...
        <div class="main-card">
            <div class="card-header-custom d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="mb-0"><i class="icon-table me-2"></i> Parentescos</h3>
                ...
                <div class="d-flex gap-2">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                            <li><a class="dropdown-item" href="../controladores/controlador_parentescos.php?action=exportar&formato=excel&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-file-excel text-success me-2"></i> Excel</a></li>
                            <li><a class="dropdown-item" href="../controladores/controlador_parentescos.php?action=exportar&formato=csv&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-file-text text-primary me-2"></i> CSV</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item" href="../controladores/controlador_parentescos.php?action=exportar&formato=txt&busqueda=<?php echo isset($_GET['busqueda']) ? urlencode($_GET['busqueda']) : ''; ?>&campo=<?php echo isset($_GET['campo']) ? urlencode($_GET['campo']) : ''; ?>"><i class="icon-doc-text-inv text-secondary me-2"></i> TXT</a></li>
                        </ul>
                    </div>
                    <?php endif; ?>
            ...
            <div class="card-body p-4">
                <!-- Buscador -->
                <form method="GET" action="vista_parentescos.php" class="mb-4">
                    <div class="input-group">
                        ...
                        <?php if(isset($_GET['busqueda']) && $_GET['busqueda'] !== ''): ?>
                            <a href="vista_parentescos.php" class="btn btn-outline-danger d-flex align-items-center"><i class="icon-cancel"></i></a>
                        <?php endif; ?>
                    </div>
                </form>
...
?>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`parentescos`.`nombre`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        nombre                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`parentescos`.`nombre`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`parentescos`.`orden`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        orden                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`parentescos`.`orden`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                                <th>
                                    <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => "`parentescos`.`estado`", 'dir' => $nextDir])); ?>" class="text-decoration-none text-muted">
                                        estado                                        <?php if (str_replace(['`',' '], '', $sort) === str_replace(['`',' '], '', "`parentescos`.`estado`")): ?>                                            <i class="icon-<?php echo ($dir === 'ASC') ? 'up-dir' : 'down-dir'; ?> ms-1"></i>
                                        <?php endif; ?>                                    </a>
                                </th>
                        ...
                        <tbody>
                            <?php
                            require_once '../modelos/modelo_parentescos.php';
                            ...
                            $modelo = new ModeloParentescos();
                            $modeloLog = new ModeloAcc_log();
                            $modeloLog->registrar($_SESSION['usuario_id'] ?? 0, 'VIEW', 'parentescos', 'Acceso a la pantalla de listado');
                     ...
                     <div class="modal-header">
                         <div class="row">
                             <div class="form-group col-md-8">
                               <h5 class="modal-title">Actualizar Parentescos - ID: </h5>
                             </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('formCrear').addEventListener('submit', function(e) {
                ...
                fetch('../controladores/controlador_parentescos.php?action=crear', {
                    method: 'POST',
            ...
            document.getElementById('formActualizar').addEventListener('submit', function(e) {
                e.preventDefault();
                const formData = new FormData(this);
                fetch('../controladores/controlador_parentescos.php?action=actualizar', {
                    method: 'POST',
                    body: new URLSearchParams(formData)
                })
                .then(response => response.json())
        ...
        function eliminar(id) {
            if (confirm('¿Estás seguro de que deseas eliminar este registro?')) {
                fetch('../controladores/controlador_parentescos.php?action=eliminar', {
                    method: 'POST',
                    headers: {
